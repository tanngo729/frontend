...
import { Table, Image, InputNumber, Space, Button, Tag } from 'antd';
...
const ProductTable = ({
  ...
  onDelete,
  onPositionChange,
  onEdit, // hàm edit được sử dụng cho nút "Sửa"
}) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => (
        <Space size="middle">
          <Button size="small" onClick={() => navigate(`/admin/products/${record._id}`)}>
            Chi tiết
          </Button>
